...
public class OddsAndEvens   // finish the class heading
{
    ...
    int[] output = new int[numEvens];

    for(int i = 0; i < array.length; i++)
    {
			if(array[i] % 2 == 0)
			{
				output[currentPosition] = array[i];
				currentPosition++;
			}
    }
    ...
    // position in tempArray that will receive the next element
		...
		// STEP 4: *********************************************************************
		return output;  // change me (what is it that you need to return?)
